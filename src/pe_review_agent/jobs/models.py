...
class ReviewInvocationRow(Base):
    __tablename__ = "review_invocations"
    __table_args__ = (
        CheckConstraint(
            "phase IN ('candidate', 'verification')", name="ck_review_invocation_phase"
        ),
        CheckConstraint("kind IN ('llm', 'tool')", name="ck_review_invocation_kind"),
        CheckConstraint(
        ...
        Index("ix_review_invocations_job", "job_id", "created_at"),
    )
    ...
    attempt_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("review_attempts.id", ondelete="CASCADE"),
        nullable=False,
    )
    input_key: Mapped[str] = mapped_column(String(64), nullable=False)
    phase: Mapped[str] = mapped_column(String(16), nullable=False)
    work_key: Mapped[str] = mapped_column(String(64), nullable=False)
